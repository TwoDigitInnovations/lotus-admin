...
export function StatusPill({ value }) {
  ...
  if (status === "active")
    return (
      <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-[#e0f2fe] text-[#078DD4]">
        Active
      </span>
    );
  if (status === "archived")
...
export function SelectColumnFilter({
  ...
  return (
    <select
      className="rounded-lg border border-slate-200 text-sm px-3 py-1.5 outline-none text-slate-700 bg-white"
      value={filterValue || ""}
...
export function indexID({ row }) {
  return <span className="text-slate-500 text-sm">{parseInt(row.id) + 1}</span>;
}
  ...
  ({ indeterminate, ...rest }, ref) => {
    ...
    return (
      <input
        type="checkbox"
        ref={resolvedRef}
        className="w-4 h-4 rounded border-slate-300 accent-[#078DD4] cursor-pointer"
        {...rest}
      />
    );
  },
  ...
  return (
    <button
      ...
      className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${
        checked ? "bg-[#078DD4]" : "bg-slate-200"
      }`}
    >
      <span
...
function Table({ columns, data, pageSize: defaultPageSize = 50, emptyComponent, onRowClick }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
      {/* ── Top bar ── */}
      <div className="flex items-center justify-between px-3 py-2 border-b border-slate-200 gap-3">
        {/* Search */}
        <div className="flex items-center gap-2 flex-1 min-w-0">
          <div className="flex items-center gap-2 bg-slate-100 rounded-lg px-3 py-1 flex-1 max-w-xs">
            <Search size={14} className="text-slate-400 shrink-0" />
            <input
              value={globalFilter || ""}
              onChange={(e) => setGlobalFilter(e.target.value || undefined)}
              placeholder="Search and filter"
              className="bg-transparent text-sm text-slate-700 outline-none placeholder-slate-400 w-full min-w-0"
            />
            {globalFilter && (
        ...
        <div className="relative shrink-0" ref={settingsRef}>
          ...
          {settingsOpen && (
            <div className="absolute right-0 top-full mt-1.5 bg-white rounded-xl shadow-xl border border-slate-100 w-60 z-50 py-2">
              {/* Sort by */}
              ...
              {/* Hide archived */}
              <div className="px-4 py-2 flex items-center justify-between border-b border-slate-100 pb-3 mb-1">
                <span className="text-sm text-slate-700">Hide archived</span>
                <Toggle checked={hideArchived} onChange={setHideArchived} />
              </div>

              {/* Columns */}
              <p className="px-4 pt-1 pb-1.5 text-[10px] font-semibold text-slate-400 uppercase tracking-wider">
                Columns
              </p>
              ...
              {draggableColumns.map((col, i) => (
                <div
                  ...
                  className={`flex items-center gap-2 px-4 py-1.5 cursor-grab select-none transition-colors ${
                    dragOver === i ? "bg-slate-100" : "hover:bg-slate-50"
                  }`}
                >
                  <GripVertical size={14} className="text-slate-300 shrink-0" />
                  <span
                    className={`flex-1 text-sm ${
                      col.isVisible ? "text-slate-700" : "text-slate-400"
                    }`}
                  >
              ...
              <tr {...hg.getHeaderGroupProps()} key={i}>
                {hg.headers.map((col, j) => (
                  <th
                    ...
                    key={j}
                    className="px-4 py-2 text-left text-xs font-medium text-slate-500 whitespace-nowrap select-none"
                  >
                ...
                return (
                  <tr
                    ...
                    className={`transition-colors ${onRowClick ? "cursor-pointer" : ""} ${
                      row.isSelected ? "bg-[#e0f2fe]/50" : "hover:bg-slate-50"
                    }`}
                  >
      ...
      <div className="flex items-center justify-between px-4 py-2.5 border-t border-slate-200">
        <span className="text-sm text-slate-500">
          {total === 0 ? "0" : `${from}–${to}`}{" "}
          {total > 0 && <span className="text-slate-400">of {total}</span>}
        </span>
        <div className="flex items-center gap-1">
          <button
